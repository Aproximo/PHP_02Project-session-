...
class SessionController extends Controller
{
    ...
    {   $current_session_id = session_id();
        ...
        $session = $repo->findBySessionId($current_session_id, $this->getUser()->getId());

        if (!$session){
            $this->saveSession($current_session_id);
        }

        $session = $repo->findAcSession($this->getUser()->getId());
...
//            }
    ...
    /**
     * @Route("/session/{id}{sl}", name="session_close", defaults={"sl" : ""}, requirements={"id" : "[1-9][0-9]*","sl":"/?"})
     * @Template()
     */
    public function closeAction($id){
        ...
        private function saveSession($id){
            ...
            $info = $this->parse_user_agent($_SERVER['HTTP_USER_AGENT']);
            ...
            $session->setOs($info['platform']." - ".$info['version']);
            $session->setBrowser($info['browser']);
            // 4) save the User!
